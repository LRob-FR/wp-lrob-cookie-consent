...
namespace LRob\CookieConsent\AutoUpdate;

final class Updater
{
    public const TRANSIENT_KEY      = 'lrob_cc_gh_release';
    public const TRANSIENT_TTL      = HOUR_IN_SECONDS;
    public const TRANSIENT_TTL_FAIL = HOUR_IN_SECONDS;
    public const PLUGIN_SLUG        = 'lrob-cookie-consent';
    ...
    public function check_for_update($transient)
    {
        ...
        if (version_compare(LROB_CC_VERSION, $remote_version, '>=')) {
            return $transient;
        }
        ...
        if ($zip_url === null) {
            ...
            // pointing WP at the GitHub source tarball (commit-hash folder name).
            return $transient;
        }

        $update = (object) [
            'slug'         => self::PLUGIN_SLUG,
            'plugin'       => LROB_CC_BASENAME,
            'new_version'  => $remote_version,
            'url'          => LROB_CC_GITHUB_URL,
            'package'      => $zip_url,
        ...
        $transient->response[LROB_CC_BASENAME] = $update;
        return $transient;
    }
    ...
    public function plugin_info($result, $action, $args)
    {
        ...
        return (object) [
            'name'          => 'LRob - Cookie Consent',
            'slug'          => self::PLUGIN_SLUG,
            'version'       => $remote_version,
            'author'        => '<a href="https://www.lrob.fr">LRob</a>',
            'homepage'      => defined('LROB_CC_PLUGIN_URL') ? LROB_CC_PLUGIN_URL : LROB_CC_GITHUB_URL,
            'requires'      => '6.8',
            'requires_php'  => '8.2',
            'tested'        => $this->tested_wp_version(),
            ...
            'sections'      => [
                'description' => __('Lean, opinionated GDPR / ePrivacy cookie consent — opt-in banner, script & iframe blocking, proof of consent.', 'lrob-cookie-consent'),
    ...
    {
        $url = defined('LROB_CC_GITHUB_URL') ? LROB_CC_GITHUB_URL : '';
        if (preg_match('#github\.com/([^/]+/[^/]+?)/?$#', $url, $m)) {
            return $m[1];
        }
        return 'LRob-FR/wp-lrob-cookie-consent';
    }
    ...
    /** Minimal Markdown → HTML for the changelog modal. */
