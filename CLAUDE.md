...
**Design stance (from `spec.md` §0):** lean (a light local scanner — but no remote cookie database, no policy generator, no packaged integrations, GDPR core only), opinionated (opt-in by default, immutable built-in categories + custom ones, no consent-per-service in v1), self-contained (no Composer, no React, no build pipeline — plain PHP 8.2+, vanilla JS, server-rendered admin). The only outbound network call is the GitHub auto-updater. Target: front JS < 15 KB, front CSS < 10 KB, total plugin far under Complianz's ~23 MB.
...
- **Layout redesign + "Custom" everywhere** — still some vertical stacking. Wants a **categories modal**, drag-reorder buttons, a detect-cookies step inside the wizard, and — the big one — **every appearance control to offer a "Custom" option** with **Gutenberg-style unit controls** (value + unit + default, validated): popup width, density, font, corners, etc. **Font fully customizable** (custom size with unit, and ideally family). (Animation/hover/edge-distance + position-under-Appearance already done.)
...
- `Frontend\{Banner,Assets,Appearance}` (§4–§5) — banner markup + the `assets/js/consent.js` state machine, public `window.lrobCc` API, category body classes, optional WP Consent API mirroring. **FSE-first appearance**: defaults inherit theme global-style tokens via `wp_get_global_settings()`. Theme Auto/Light/Dark + Midnight/Ocean/Sand palettes/Custom; **every color a `.lrob-cc-banner` CSS variable** incl. button hover (custom or `color-mix` auto-darken). `Appearance::inline_css()` emits all the vars: width/density/font/radius, alignments, **edge offsets** (preset or custom value+unit, also on `<body>` so the floating revisit button follows them), **entrance animation** (fade/slide/zoom composed from one keyframe), logo height. `show_delay` (default 1000ms, first appearance only). Style + text presets (`Support\Presets`, translatable; filterable). **Placeholders**: iframes get a sized click-to-load placeholder; **CAPTCHAs** (Turnstile/reCAPTCHA/hCaptcha) get one inside their own container (`Assets::captcha_blocks()` → selector+category; `lrob_cc_captcha_providers` filter). Clicking "Accept & load" is a **provisional grant** (persists the category, keeps the banner asking until a formal decision; banner shows until the `lrob_cc_status` cookie is `dismissed`). **Server-side blocking is consent-independent** (always emits `data-src`); activation is client-side → cache-safe.
- `Consent\{Schema,LogRepository,RestController,BannerVersion}` (§6) — legally-robust proof of consent (schema **v5**, migrates via `Schema::maybe_upgrade()` incl. an `ip_anon`→`ip` ALTER). One row per event: anonymous `consent_id` (subject), granular per-purpose `choices` JSON (**only categories actually proposed** — `Rules::active_categories()`), `method`+`payload`, `event_type` (consent/update/withdraw), `banner_version` → snapshot, `expires_at` (~13mo), `ip` (`ip_storage` hashed default/full), opt-in UA + `user_id`. **`BannerVersion` is captured from the rendered banner DOM at consent time** (`record()` — the actual translated, active-only on-screen text), not server `__()` (which would be base-language under TranslatePress); server `snapshot()` is a fallback. TranslatePress `#!trpst#…#!trpen#` markers stripped (`clean()`). Logging **on by default**. `keep_data_on_uninstall` (default on) preserves the proof tables on uninstall. Admin: `Admin\ConsentLogTable` (`WP_List_Table` — Date/Visitor ID/IP/WP user/Event/Act/Choices/User agent/Version/Renew by; per-row/bulk delete, CSV).
...
- `Support\Categories` — `functional` is hardcoded/forced-on. Optional categories = **immutable built-in defaults** (`preferences/statistics/marketing/embed/security`, computed — NOT stored) + **custom** ones (`lrob_cc_options['categories']` holds customs only, so new built-ins appear for everyone and old saves can't freeze them out). Built-in **names** are fixed (translate via gettext) but their **descriptions are editable** (`cat_desc_overrides`). Everything iterates `Categories::optional()`/`labels()` — never hardcode. **`Rules::active_categories()`** = optional categories that actually block something; that subset is what the banner offers and the log records (don't ask/record consent for empty categories). Rules referencing an invalid category are dropped.
